...
class BasicFitter(object):
    ...
    def fit(self, range_low_lim=0, range_up_lim=None):
        if range_up_lim is None:
            range_up_lim = max(self.radii)
        good_inds = np.where((self.radii > range_low_lim) * (self.radii < range_up_lim))
        def chi_sq(params):
            self.set_params(params)
            diffs = ((self.surf_bri - self.evaluate()) / self.surf_bri_std)**2
            #print(diffs)
            self.chi = np.sum(diffs[good_inds])
            self.chi *= self.chi_penalty()
            return self.chi
        fmin(chi_sq, x0=self.get_par_values(), disp=False)
        self.fit_done = True
        penalty = self.bic_penalty()
        self.bic = bic(self.surf_bri[good_inds], self.surf_bri_std[good_inds],
                       self.evaluate()[good_inds], 2) + penalty
...
class SimpleDiskFitter(BasicFitter):
    ...
    def evaluate(self, unpack=False):
        ...
        # print(np.max(-2.5 * np.log10(flux_convolved) + m0))
...
class BulgeSimpleDiskFitter(BasicFitter):
    ...
    def evaluate(self, unpack=False):
        ...
        week_bulge_idx = np.where(bulge < 1e-5)[0]
        if len(week_bulge_idx):
            bulge[week_bulge_idx[0]:] = 1e-5
        ...
        if unpack is False:
            ...
            return (bulge, disk, total, week_bulge_idx[0])
...
def make_n_break_fitter(n_breaks):
    """
    Function returns a class that represents a disk with N breaks
    """
    class NBrokenDiskFitter(BasicFitter):
        name = f"Expdisk with {n_breaks} breaks"
        ref_points = ["Point on first disk"]
        for i in range(n_breaks):
            ref_points.append(f"Break point {i+1}")
        ref_points.append("Point on last disk")
        n_free_params = 2 + 2 * n_breaks
        ...
        def __init__(self, radii, surf_bri, surf_bri_std, psf):
            ...
            for i in range(n_breaks+1):
                par_names.append(f"mu0_d{i+1}")
                par_dims.append("mag/sq.''")
                if i == 0:
                    par_free.append(True)
                else:
                    par_free.append(False)
            for i in range(n_breaks+1):
                par_names.append(f"h_d{i+1}")
                par_dims.append("''")
                par_free.append(True)
            for i in range(n_breaks):
                par_names.append(f"r_break{i+1}")
                par_dims.append("''")
                par_free.append(True)
            super().__init__(radii, surf_bri, surf_bri_std, psf, par_names, par_dims, par_free)

        def evaluate(self, unpack=True):
            if None in self.get_par_values():
                return None
            mag = np.zeros_like(self.radii)
            for disk_idx in range(n_breaks+1):
                if disk_idx == 0:
                    portion = np.where(self.radii < self.par_values["r_break1"])
                elif disk_idx == n_breaks:
                    portion = np.where(self.radii > self.par_values[f"r_break{n_breaks}"])
                else:
                    r1 = self.par_values[f"r_break{disk_idx}"]
                    r2 = self.par_values[f"r_break{disk_idx+1}"]
                    portion = np.where((r1 < self.radii) * (self.radii <= r2))
                mu0 = self.par_values[f"mu0_d{disk_idx+1}"]
                hd = self.par_values[f"h_d{disk_idx+1}"]
                mag[portion] = mu0 + kappa * self.radii[portion] / hd
            m0 = 20
            flux_orig = 10 ** (0.4 * (m0 - mag))
            flux_orig_symmetric = np.append(flux_orig[::-1], flux_orig)  # Make symmetric for the padding purposes
            flux_convolved = fftconvolve(flux_orig_symmetric, self.psf, mode="same")[len(flux_orig):]
        ...
        def compute_initials(self, ref_points):
            ...
            for i in range(len(ref_points)-1):
                h = kappa * (ref_points[i][0] - ref_points[i+1][0]) / (ref_points[i][1] - ref_points[i+1][1])
                h_initials.append(h)
            mu0_d1 = ref_points[0][1] - kappa * ref_points[0][0] / h_initials[0]
            r_break_initials = [ref_points[i][0] for i in range(1, len(ref_points)-1)]
            all_initials = [mu0_d1]
            all_initials.extend(h_initials)
            all_initials.extend(r_break_initials)
            self.set_params(all_initials)

        def compute_non_free(self):
            for disk_idx in range(1, n_breaks+1):
                mu = self.par_values[f"mu0_d{disk_idx}"]
                r1 = self.par_values[f"r_break{disk_idx}"]
                h1 = self.par_values[f"h_d{disk_idx}"]
                h2 = self.par_values[f"h_d{disk_idx+1}"]
                self.par_values[f"mu0_d{disk_idx+1}"] = mu + kappa * r1 * (1/h1-1/h2)

        def get_inner_disk_mu0(self):
            return self.par_values["mu0_d1"]

    return NBrokenDiskFitter


def make_n_break_fitter_with_bulge(n_breaks):
    """
    Function returns a class that represents a disk with N breaks
    """
    class BulgeNBrokenDiskFitter(BasicFitter):
        name = f"Bulge + Expdisk with {n_breaks} breaks"
        ref_points = ["First point on bulge", "Second point on bulge", "Third point on bulge", "Point on first disk"]
        for i in range(n_breaks):
            ref_points.append(f"Break point {i+1}")
        ref_points.append("Point on last disk")
        n_free_params = 5 + 2 * n_breaks

        def __init__(self, radii, surf_bri, surf_bri_std, psf):
            par_names = ["re_b", "mu0_b", "n_b"]
            par_dims = ["''", "mag/sq.''", ""]
